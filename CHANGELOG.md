...
- Generated submission-extension XML docs no longer leak an internal tracking
  reference into consumer output.
...
- `Daml.Codegen.CSharp.Model.DarModel` and `Daml.Codegen.CSharp.Model.IDarSource` — the emitter input contract. `CSharpCodeGenerator.Generate` now takes `IDarSource`, satisfied by both `DarModel` (proto-direct) and `DarArchive` (parser-direct).
...
- **Package split, per a decision kept in the project's internal ADR collection.** `Daml.Codegen.CSharp` is now a pure emitter library — it consumes an `IntermediateDar` proto (produced by the JVM helper) and emits `.cs`. The legacy `dotnet tool` CLI surface (`PackAsTool`, `daml-codegen-csharp` command) is removed from `Daml.Codegen.CSharp`. DAR/DALF parsing moves behind the `IDarSource` abstraction, out of the `Daml.Codegen.CSharp` emitter library. A new thin CLI project `Daml.Codegen.CSharp.Cli` produces the binary `daml-codegen-csharp`; it accepts either `--intermediate <proto-path>` (the primary path) or one-or-more positional `<dar-files>` (legacy direct path retained for local dev). The CLI publishes as a self-contained single-file native binary per RID — `dotnet publish src/Daml.Codegen.CSharp.Cli -c Release -r <rid> --self-contained true -p:PublishSingleFile=true`.
...
- **Cross-package choice argument types now resolve to their fully-qualified
  ...
  the wire. The defensive filters added for the non-CID exerciser and the
...
- **`Daml.Runtime.Stdlib.GenericStub.NotImplemented<T>(string)`** — runtime stub
  ...
  for proper static-abstract dispatch.
...
- **Variant `FromRecord` stub**. Variants are emitted with a `FromRecord` that
  ...
  still compile. Full variant codec support is planned.
...
[Unreleased]: https://github.com/peacefulstudio/daml-codegen-csharp/compare/v0.1.8-preview.4...HEAD
[0.1.8-preview.4]: https://github.com/peacefulstudio/daml-codegen-csharp/compare/v0.1.8-preview.3...v0.1.8-preview.4
