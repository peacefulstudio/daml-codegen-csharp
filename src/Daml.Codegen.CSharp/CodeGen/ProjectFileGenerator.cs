    ...
    private string FormatPackageVersion(Version darVersion)
    {
        if (darVersion.Build < 0)
        {
            throw new InvalidOperationException(
                $"Daml package version must be 3-part (Major.Minor.Patch) to produce a 4-part NuGet version per ADR 0002, " +
                $"but got '{darVersion}'. The IntermediateDarReader and DarArchive paths both guarantee a 3-part version; " +
                $"a 2-part version here indicates a regression in the upstream parser.");
        }
        if (_options.EmitterCounter < 0)
        {
            throw new InvalidOperationException(
                $"CodeGenOptions.EmitterCounter is the monotonic 4th segment of M.m.p.r per ADR 0002; " +
