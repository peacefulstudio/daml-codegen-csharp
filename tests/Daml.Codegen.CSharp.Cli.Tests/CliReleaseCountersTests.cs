...
public class CliReleaseCountersTests : IDisposable
{
    ...
    public async Task release_counters_flag_fails_loudly_when_intermediate_is_not_provided()
    {
        ...
        exit.Should().NotBe(0,
            "the content hash that keys the counter store is only computable from the IntermediateDar proto; --release-counters without --intermediate must fail rather than silently emit r=0 against a different content baseline");
    }
