# `splice-api-token-allocation-instruction-v2` codegen snapshot
...
The Token Standard V2 AllocationInstruction / AllocationFactory interface
package (CIP-0112).
...
```bash
scripts/refresh-snapshot.sh splice-api-token-allocation-instruction-v2
```

## The vendored inputs

`intermediate.binpb` is the canonical codegen input for the drift test;
`splice-api-token-allocation-instruction-v2.dar` is the upstream Splice
archive it was derived from
(`splice-api-token-allocation-instruction-v2-1.0.0.dar` from the Splice
`0.7.5` `splice-node` release tarball), kept alongside as the frozen upstream
artifact and used as the regeneration input. Do not hand-edit
