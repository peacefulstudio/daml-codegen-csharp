...
`splice-test-token-v1.dar` is the upstream Splice archive it was derived
from (`splice-test-token-v1-1.0.0.dar` from the Splice `0.6.13`
...
Splice package genuinely needs to advance, replace the files in place,
refresh the `expected/` snapshot per the procedure above, and call out the
version bump in the pull request description.
