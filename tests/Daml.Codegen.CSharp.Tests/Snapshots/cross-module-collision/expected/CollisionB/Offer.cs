...
) : ITemplate, IDamlRecord<Offer>
{
    ...
    public sealed record Contract(ContractId Id, Offer Data) : IContract<ContractId, Offer>
    {
        ...
        public static Contract FromCreatedEvent(CreatedEvent @event) =>
            new(new ContractId(@event.ContractId), global::CollisionB.Offer.FromRecord(@event.CreateArguments));
    }
}
...
)
{
    ...
    public static ExerciseOutcome<RetagResult> FromCreatedContracts(IEnumerable<CreatedContract> created)
    {
        ...
        {
            if (string.Equals(item.TemplateId.ModuleName, global::CollisionB.Offer.TemplateId.ModuleName, StringComparison.Ordinal)
                && string.Equals(item.TemplateId.EntityName, global::CollisionB.Offer.TemplateId.EntityName, StringComparison.Ordinal))
            {
        ...
        if (matches0.Count > 1)
        {
            return new ExerciseOutcome<RetagResult>.Many(EquatableArray.Create(matches0));
        }

        return new ExerciseOutcome<RetagResult>.One(new RetagResult(
            Offer: new ContractId<global::CollisionB.Offer>(matches0[0])
        ));
    }
