# `splice-api-token-transfer-events-v2` codegen snapshot
...
The Token Standard V2 transfer-events package — V2-only, with no V1
counterpart (a novel shape). Pinning it guards the emitter against
regressions on a package unlike any other vendored fixture.
...
```bash
scripts/refresh-snapshot.sh splice-api-token-transfer-events-v2
```

## The vendored inputs

`intermediate.binpb` is the canonical codegen input for the drift test;
`splice-api-token-transfer-events-v2.dar` is the upstream Splice archive it
was derived from (`splice-api-token-transfer-events-v2-1.0.0.dar` from the
Splice `0.7.5` `splice-node` release tarball), kept alongside as the frozen
upstream artifact and used as the regeneration input. Do not hand-edit
